...
def open_csv_file(filepath, intensity=True):
    # Read the Excel file into a DataFrame
    df = pd.read_csv(filepath)

    # Extract necessary columns
    scan_numbers = df['scan_number'].tolist()
    reducing_mass = df['reducing_mass'].tolist()
    precursor_charges = df['charge'].tolist()
    rts = df['RT'].tolist()
    intensities = df['intensity'].tolist() if intensity else None

    # Parse the 'peak_d' column
    peak_d_values = df['peak_d'].apply(eval).tolist()
...
def load_file(filepath, intensity=True):
    if filepath.endswith(".mzML"):
        ...
        return open_csv_file(filepath, intensity)
    else:
        raise ValueError("Unsupported file format. Supported formats: .mzML, .mzXML, .csv")
